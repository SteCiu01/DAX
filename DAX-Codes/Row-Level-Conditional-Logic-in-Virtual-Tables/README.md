...
If until the currency level is there, the measure Logic works as intended (although no totals are displayed) but when we roll up at child id level then it does not return the intended number, while the Old Position (Correc) does. This because the latter still calculates the net position at currency level and then sums up  these positions for each level of the hierarchy. 
